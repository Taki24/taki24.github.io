...
(levels, options={}) => {
  ...
  var layout = {
    width: d3.max(nodes, n => n.x) + node_width + 2 * padding + 400,
    height: d3.max(nodes, n => n.y) + node_height / 2 + 2 * padding + 100,
    node_height,
    node_width,
    bundle_width,
    level_y_padding,
  ...
  input.addEventListener('change', () => {
    readXlsxFile(input.files[0]).then(rows => {
        ...
        for(let i = 1; i < rows.length; i++){
            const row = rows[i];
            const firstElement = row[0] + "-" + row[1] + "-" + row[2]; 
            temp.push([firstElement, ...row.slice(3, row.length)]);
        }        
        
        const header = temp[0];
        header.shift();
        const parents = [];
        header.map(parent => parents.push({id: parent}));
        // console.log(header)

        for(let i = 0; i < temp.length; i++){
            for(let j = 0; j < temp[i].length ; j++){
                if(temp[i][j] !== null && temp[i][j].toLowerCase() === 'x'){
                    let exists = false;
                    result.map(dep => {
                        if(dep.id === temp[i][0]){                            
                            dep.parents.push(header[j-1]);
                            exists = true;
                        }
                    });
                    if(!exists){
                      result.push({id: temp[i][0], parents: [header[j-1]]});                      
                    }
                }
            }
        }            
        finalResult.push(parents);
